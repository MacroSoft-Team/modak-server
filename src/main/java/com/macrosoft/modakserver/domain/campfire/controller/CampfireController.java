...
public class CampfireController {
    ...
    @GetMapping(CAMPFIRE_PIN_URI + "/join/info")
    public BaseResponse<CampfireResponse.CampfireJoinInfo> getCampfireJoinInfo(
            @Parameter(description = CAMPFIRE_PIN_DESCRIPTION, example = CAMPFIRE_PIN_EXAMPLE)
            @PathVariable(CAMPFIRE_PIN) int campfirePin
    ) {
        return BaseResponse.onSuccess(
                campfireService.getCampfireJoinInfo(campfirePin));
    }
